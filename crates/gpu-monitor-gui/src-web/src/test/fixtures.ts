...
export function gpu(index = 0, at = TIME): GpuInfo {
    return {
        ...
        metrics: { gpu_utilization: 50, memory_utilization: 20, encoder_utilization: 0, decoder_utilization: 0, temperature: 65, power_usage: 125000, fan_speed: 35, clock_graphics: 1500, clock_memory: 7000, clock_sm: 1500, performance_state: 'P0', throttle_reasons: [], pcie_generation: 4, pcie_width: 16, pcie_rx_kb_per_second: 0, pcie_tx_kb_per_second: 0 },
...
export function snapshot(at = TIME, gpus = [gpu(0, at)], error: SampleError | null = null): MonitorSnapshot {
    return { schema_version: 1, sampled_at_ms: at, gpus, failures: [], error };
}
